...
public class jdbcpostgreSQL {
  ...
  public static Integer[] getInventory(int orderedgyro, int orderedbowl, int orderedpitahummus, int orderedfalafel,
      int orderedChicken, int orderedMeat, int orderedharissa, int orderedBalsamic, int orderedtza, int ordereddrink) {
    ...
    while (bowlOrGyro > 0) {
      ...
      // Dressing
      int dressingCount = getRandomValue(1, 2);
      switch (dressingCount) {
        case 1:
          inventory[13] += 1;
          break;
    ...
    inventory[1] += orderedChicken;
    inventory[2] += orderedMeat;
  

    inventory[13] += orderedharissa;
        
    inventory[14] += orderedtza;
    
    inventory[15] += orderedBalsamic;
  ...
  public static void main(String args[]) {
    ...
    try {
      ...
      for( int i=0; i< orderIdArray.length; i++){ 
        ...
        Integer[] orderedItems = new Integer[10];
        ...
        while (orderCount <  numOrdersWant ) { 
          int checkoutid = 0;
          int orderedgyro = 0;
          int orderedbowl = 0;
          int orderedpitahummus = 0;
          ...
          int orderedChicken = getRandomValue(0, 1);
          int orderedMeatball = getRandomValue(0, 1);

          int orderedTza = getRandomValue(0, 1);
          int orderedharissa = getRandomValue(0, 1);
          int balsamic = getRandomValue(0, 1);

          ordereddrink = getRandomValue(0, 2);
          int orderedprotein = orderedChicken + orderedMeatball; 
          int ordereddressing = orderedharissa + balsamic + orderedTza;
          // orderedharissa, int orderedBalsamic, int orderedtza
          amount = getAmount(orderedgyro, orderedbowl, orderedpitahummus, orderedfalafel,
          orderedprotein, ordereddressing, ordereddrink); 

          Integer[] inventoryArr = getInventory(orderedgyro, orderedbowl, orderedpitahummus, orderedfalafel,
          orderedChicken, orderedMeatball, orderedharissa,balsamic, orderedTza, ordereddrink);

          orderedItems[0] = orderedbowl;
          orderedItems[1] = orderedfalafel;
          orderedItems[2] = ordereddrink;
          orderedItems[3] = orderedharissa; 
          orderedItems[4] = orderedMeatball; 
          orderedItems[5] = orderedTza; 
          orderedItems[6] = balsamic;
          orderedItems[7] = orderedpitahummus;
          orderedItems[8] = orderedChicken;
          orderedItems[9] = orderedgyro;
          ...
          PreparedStatement statement = conn.prepareStatement(
              "INSERT INTO ordering(orderid , timeoforder , amount ,  ordereditems, inventory ) VALUES (?,?,?, ?, ?)");

          // transform java data into proper SQL variables
          
          Array ordered = conn.createArrayOf("INT", orderedItems);
          statement.setInt(1, orderId);
          statement.setTime(2, time);
          statement.setDouble(3, amount);
          statement.setArray(4, ordered);
          Array inventory = conn.createArrayOf("INT", inventoryArr);
          statement.setArray(5, inventory);
